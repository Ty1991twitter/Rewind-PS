...
express.listen(port, () => {
    console.log("RewindPS started listening on port", port);
...
try {
    if (!fs.existsSync(path.join(process.env.LOCALAPPDATA, "RewindPS"))) fs.mkdirSync(path.join(process.env.LOCALAPPDATA, "RewindPS"));
} catch (err) {
    // fallback
...
express.use((req, res, next) => {
    var XEpicErrorName = "errors.com.RewindPS.common.not_found";
